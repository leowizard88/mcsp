...
export async function onRequestGet({ request, env }) {
  ...
  const messages = (await readMessages(env)).map(safeMessage).filter(m => m.text).slice(-80);
...
export async function onRequestPost({ request, env }) {
  ...
  messages.push(msg);
  await env.CHAT_MESSAGES.put(chatKey, JSON.stringify(messages.slice(-80)));
  return json({ message:msg, messages:messages.slice(-80).map(safeMessage) });
}
