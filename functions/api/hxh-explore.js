...
const characterKey = u => `hxh:character:${u.id}`;
const exploreKey = u => `hxh:explore:${u.id}`;
...
const maxHealthFor = c => {
  const p = effectiveParams(c), level = clampInt(c?.level, 1);
...
const saveCharacter = (env, user, c) => env.CHAT_MESSAGES.put(characterKey(user), JSON.stringify({ ...normalizeLevelXp(c), updatedAt:new Date().toISOString() }));
...
function pushNamed(list, obj) { list.push(obj?.name || obj?.nome || obj || 'sconosciuto'); }
const ENEMIES = {
  goblin:{ key:'goblin', name:'Goblin', hp:7, damage:3, exp:[1,3], jenny:[0,900], miss:50, dodge:5, loot:[
    { p:60, card:() => freeCard('goblin-card', 'Carta Goblin', 'E', 'La carta di un goblin schifoso forse vale qualche Jenny', false) },
  ...
  goat:{ key:'goat', name:'Vecchia Capra', hp:8, damage:4, exp:[1,3], jenny:[0,450], miss:60, dodge:0, loot:[{ p:50, card:() => freeCard('goat-card', 'Carta Capra', 'E', 'La carta di una capra qualunque, forse qualche mercante potrebbe essere interessato', false) }]},
...
function snapshot(state, atSec) { state.snapshots.push({ atSec:Math.floor(atSec), health:structuredClone(state.health), cards:structuredClone(state.cards), inventory:structuredClone(state.inventory), xp:state.xp, jenny:state.jenny, critical:state.critical }); }
...
function simulateCombat(logs, state, atSec) {
  const enemy = enemyByPool(state), p = effectiveParams(state.character); let enemyHp = enemy.hp;
  ...
  for (let round = 1; round <= 20 && enemyHp > 0 && !state.critical; round++) {
    ...
    if (chance(enemy.miss)) addLog(logs, atSec, `${enemy.name} attacca ma manca il colpo.`, 'info');
      ...
      const part = rollWeighted(bodyWeights), entered = enemyHitDamage(enemy.damage, p.robustezza);
  ...
  state.xp += exp; state.jenny += jenny; state.summary.exp += exp; state.summary.jenny += jenny;
...
function minuteEvent(logs, state, atSec) {
  ...
  if (r < 94) {
    ...
    const stolen = pick(cards); state.cards = state.cards.filter(c => c.id !== stolen.id); pushNamed(state.summary.cardsLost, stolen);
    addLog(logs, atSec, `Un furfante ti ha rubato la carta ${stolen.name || stolen.nome}!`, 'bad'); snapshot(state, atSec); return;
  }
  const spell = addCard(state.cards, spellCard(`Incantesimo del viaggiatore ${rollInt(1,99)}`, chance(78) ? 'E' : 'D'));
  state.rewardCards.push(spell); pushNamed(state.summary.cardsGained, spell);
  addLog(logs, atSec, `Incontri un viaggiatore misterioso che lascia cadere per terra ${spell.name}!`, 'good'); snapshot(state, atSec);
}
function rollBonus(diffKey, inventory) {
  const b = DIFFICULTY[diffKey].bonus, total = b.common + b.uncommon + b.rare + b.epic + b.none;
  let n = Math.floor(rnd() * total) + 1;
  for (const [key,label,weight] of [['common','comune',b.common],['uncommon','non comune',b.uncommon],['rare','raro',b.rare],['epic','epico',b.epic],['none','nulla',b.none]]) {
    n -= weight;
    if (n <= 0) {
      ...
      const name = pick(names[key]);
      const item = { id:`bonus-${key}-${Date.now()}-${Math.random().toString(16).slice(2)}`, type:'item', name, nome:name, rarity:label, rarita:label, description:`Oggetto ${label} trovato nel modulo bonus.`, descrizione:`Oggetto ${label} trovato nel modulo bonus.`, createdAt:new Date().toISOString() };
...
function buildExploration(character, mode) {
  ...
  const diffKey = LOCATION_DIFFICULTY[location] || 'nabbo', diff = DIFFICULTY[diffKey], p = effectiveParams(character);
  const durationMin = Math.max(5, diff.minutes - clampInt(p.agilita)), moduleMin = Math.max(1, Math.floor(durationMin / 5)), remainderMin = durationMin - moduleMin * 5;
  const enemyChance = mode === 'sicura' ? Math.max(10, diff.enemyChance - clampInt(p.percezione) * 2) : diff.enemyChance;
  const logs = [], startHealth = normalizeHealth(character), startGeneral = generalHealth(startHealth);
  const state = { character, health:structuredClone(startHealth), maxHealth:maxHealthFor(character), cards:[...(Array.isArray(character.cards) ? character.cards : [])], inventory:[...(Array.isArray(character.inventory) ? character.inventory : [])], rewardCards:[], xp:clampInt(character.xp), jenny:clampInt(character.jenny), delay:0, critical:false, criticalAtSec:null, lastEnemyKey:null, snapshots:[], summary:{ killed:{}, jenny:0, exp:0, cardsGained:[], itemsGained:[], healthLost:0, cardsLost:[], itemsLost:[] } };
  snapshot(state, 0);
  ...
  for (let block = 0; block < Math.floor(durationMin / 10); block++) {
    const used = [];
    for (let i = 0; i < rollAccidentCount(diffKey); i++) {
      let sec = block * 600 + rollInt(1, 599), tries = 0;
      while (used.some(x => Math.abs(x - sec) < 130) && tries++ < 20) sec = block * 600 + rollInt(1, 599);
      used.push(sec); events.push({ t:sec, type:'accident' });
    }
  }
  ...
  for (const ev of events) {
    ...
    if (ev.type === 'enemy') chance(enemyChance) ? simulateCombat(logs, state, at) : addLog(logs, at, 'Nessun nemico incontrato nel modulo.', 'info');
    if (ev.type === 'accident') {
  ...
  if (!state.critical && remainderMin > 0) {
    ...
    else addLog(logs, totalSeconds, 'Modulo bonus: non hai trovato nulla.', 'info');
...
async function applyProgress(env, user, character, exploration) {
  const view = visibleExploration(exploration); const snap = latestSnapshot(exploration, view?.elapsedSeconds || 0);
  if (!snap || clampInt(snap.atSec) <= clampInt(exploration.appliedSnapshotAt)) return normalizeLevelXp(character);
  let next = { ...character, health:snap.health || character.health, cards:snap.cards || character.cards || [], inventory:snap.inventory || character.inventory || [], xp:clampInt(snap.xp), jenny:clampInt(snap.jenny), activeExploration: view?.done ? null : character.activeExploration };
  ...
  next = normalizeLevelXp(next); await saveCharacter(env, user, next);
...
export async function onRequestGet({ request, env }) {
  ...
  let character = await readCharacter(env, user); if (!character) return json({ error:'Crea prima un personaggio HxH' }, 404);
  let exploration = await readExplore(env, user); character = normalizeLevelXp(character); if (exploration) character = await applyProgress(env, user, character, exploration); exploration = await readExplore(env, user);
...
export async function onRequestPost({ request, env }) {
  ...
  const action = clean(data.action).toLowerCase(); let character = normalizeLevelXp(await readCharacter(env, user)); if (!character) return json({ error:'Crea prima un personaggio HxH' }, 404);
  ...
  if (action === 'start') {
    ...
    const mode = clean(data.mode).toLowerCase() === 'scoperta' ? 'scoperta' : 'sicura'; const location = clean(character.location || 'Shiso tree'), diffKey = LOCATION_DIFFICULTY[location] || 'nabbo', cost = DIFFICULTY[diffKey].energy;
    if (clampInt(character.energy) < cost) return json({ error:`Energia insufficiente: servono ${cost} punti energia.` }, 403);
    const exp = buildExploration(character, mode); character.energy = clampInt(character.energy) - cost; character.energyUpdatedAt = new Date().toISOString(); character.activeExploration = { id:exp.id, location:exp.location, startedAt:exp.startedAt, totalSeconds:exp.totalSeconds };
    await saveCharacter(env, user, character); await saveExplore(env, user, exp); return json({ character, exploration:visibleExploration(exp) });
  }
  ...
  if (action === 'claim') { if (!visibleExploration(exploration).done) return json({ error:'Esplorazione ancora in corso.' }, 403); if (exploration.final?.critical) return json({ error:'Esplorazione cancellata: sei in stato critico.' }, 403); exploration.claimed = true; exploration.status = 'claimed'; character.activeExploration = null; await saveCharacter(env, user, character); await saveExplore(env, user, exploration); return json({ character, exploration:visibleExploration(exploration), claimed:true }); }
