...
const saveCharacter = (env, user, c) => env.CHAT_MESSAGES.put(characterKey(user), JSON.stringify({ ...c, updatedAt:new Date().toISOString() }));
...
const rollWeighted = weights => {
  const entries = Object.entries(weights), total = entries.reduce((a,[,w]) => a + w, 0);
  let n = rnd() * total;
...
function addCard(cards, card) {
  const c = { ...card };
  if (c.cardType === 'free' && !(c.slot || c.number)) c.slot = c.number = freeSlot(cards);
  if (c.cardType === 'spell' && !(c.slot || c.number)) c.slot = c.number = spellSlot(cards);
  cards.push(c);
  return c;
}
const ENEMIES = {
  goblin:{ key:'goblin', name:'Goblin', hp:7, damage:3, exp:[1,3], jenny:[0,1000], miss:50, dodge:5, loot:[
  ...
  goat:{ key:'goat', name:'Vecchia Capra', hp:8, damage:4, exp:[1,3], jenny:[0,500], miss:60, dodge:0, loot:[
    { p:100, card:() => freeCard('goat-card', 'Carta Capra', 'E', 'La carta di una capra qualunque, forse qualche mercante potrebbe essere interessato', false) }
  ]},
  bossGoblin:{ key:'bossGoblin', name:'Capo Goblin', hp:12, damage:3, exp:[5,10], jenny:[2000,4000], miss:20, dodge:30, loot:[
...
function enemyByPool(state) {
  let roll = rollInt(1, 100);
  let enemy = roll <= 60 ? ENEMIES.goblin : roll <= 90 ? ENEMIES.goat : ENEMIES.bossGoblin;
  if (state.lastEnemyKey === 'bossGoblin' && enemy.key === 'bossGoblin') {
    roll = rollInt(1, 90);
    enemy = roll <= 60 ? ENEMIES.goblin : ENEMIES.goat;
  }
  state.lastEnemyKey = enemy.key;
  return enemy;
}
function playerDamage(character, health) {
  const p = effectiveParams(character), inv = Array.isArray(character.inventory) ? character.inventory : [];
...
function simulateCombat(logs, state, atSec) {
  ...
  if (enemyHp > 0) return addLog(logs, atSec, `${enemy.name} si ritira nel buio della Foresta Oscura.`, 'info');
  const exp = rollInt(enemy.exp[0], enemy.exp[1]), jenny = rollInt(enemy.jenny[0], enemy.jenny[1]);
  state.exp += exp; state.jenny += jenny;
  addLog(logs, atSec, `${enemy.name} è esausto. Ottenuti ${exp} EXP e ${jenny} Jenny.`, 'good');
  for (const loot of enemy.loot) if (chance(loot.p)) { const card = addCard(state.cards, loot.card()); state.rewardCards.push(card); addLog(logs, atSec, `Ottenuta ${card.name || card.nome}!`, 'good'); }
}
function minuteEvent(logs, state, atSec) {
  ...
  if (r < 84) {
    ...
    const stolen = pick(cards); state.cards = state.cards.filter(c => c.id !== stolen.id);
    return addLog(logs, atSec, `Un furfante ti ha rubato la carta ${stolen.name || stolen.nome}!`, 'bad');
  }
  const spell = addCard(state.cards, spellCard(`Incantesimo del viaggiatore ${rollInt(1,99)}`, chance(75) ? 'E' : 'D'));
  state.rewardCards.push(spell); addLog(logs, atSec, `Incontri un viaggiatore misterioso che lascia cadere per terra ${spell.name}!`, 'good');
}
...
function rollAccidentCount(diffKey) {
  const weights = DIFFICULTY[diffKey].accidents, total = weights.reduce((a,b) => a + b, 0);
  let n = rnd() * total;
...
function buildExploration(character, mode) {
  ...
  const logs = [], state = { character, health:normalizeHealth(character), maxHealth:maxHealthFor(character), cards:[...(Array.isArray(character.cards) ? character.cards : [])], inventory:[...(Array.isArray(character.inventory) ? character.inventory : [])], rewardCards:[], exp:0, jenny:0, delay:0, critical:false, criticalAtSec:null, lastEnemyKey:null };
  ...
  for (const ev of events) {
    ...
    if (ev.type === 'accident') {
      ...
      state.delay += 120; protectedUntil = at + 120; addLog(logs, protectedUntil, 'Sei uscito dalla trappola. L’esplorazione riprende.', 'info');
  ...
  if (!state.critical && remainderMin > 0) {
    ...
    item ? addLog(logs, totalSeconds, `Modulo bonus: trovato ${item.name} (${item.rarity}).`, 'good') : addLog(logs, totalSeconds, 'Modulo bonus: non hai trovato nulla.', 'info');
  }
...
async function applyCriticalIfNeeded(env, user, character, exploration) {
  ...
  const health = normalizeHealth(character); health.testa = 0;
  ...
  exploration.status = 'critical'; await saveExplore(env, user, exploration);
  return next;
}
async function claim(env, user, character, exploration) {
  ...
  const next = { ...character, health:exploration.final.health || normalizeHealth(character), cards:exploration.final.cards || character.cards || [], inventory:exploration.final.inventory || character.inventory || [], xp:clampInt(character.xp) + clampInt(exploration.final.exp), jenny:clampInt(character.jenny) + clampInt(exploration.final.jenny), activeExploration:null, updatedAt:new Date().toISOString() };
  await saveCharacter(env, user, next);
  exploration.claimed = true; exploration.status = 'claimed'; await saveExplore(env, user, exploration);
  return next;
}
...
export async function onRequestPost({ request, env }) {
  ...
  if (action === 'die') {
    ...
    await env.CHAT_MESSAGES.delete(characterKey(user)); await deleteExplore(env, user); return json({ dead:true });
  }
  ...
  if (action === 'start') {
    ...
    character.energy = clampInt(character.energy) - cost; character.energyUpdatedAt = new Date().toISOString(); character.activeExploration = { id:exp.id, location:exp.location, startedAt:exp.startedAt, totalSeconds:exp.totalSeconds }; character.updatedAt = new Date().toISOString();
  ...
  character = await applyCriticalIfNeeded(env, user, character, exploration); exploration = await readExplore(env, user);
  if (action === 'claim') { try { character = await claim(env, user, character, exploration); } catch (err) { return json({ error:err.message }, 403); } return json({ character, exploration:visibleExploration(exploration), claimed:true }); }
