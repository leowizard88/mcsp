...
function snapshot(state, atSec) {
  state.snapshots.push({ atSec:Math.floor(atSec), health:structuredClone(state.health), cards:structuredClone(state.cards), inventory:structuredClone(state.inventory), xp:state.startXp, jenny:state.startJenny, critical:state.critical });
}
...
function buildExploration(character, mode) {
  ...
  const state = { character, health:structuredClone(startHealth), maxHealth:maxHealthFor(character), cards:[...(Array.isArray(character.cards) ? character.cards : [])], inventory:[...(Array.isArray(character.inventory) ? character.inventory : [])], rewardCards:[], startXp:clampInt(character.xp), startJenny:clampInt(character.jenny), rewardXp:0, rewardJenny:0, delay:0, critical:false, criticalAtSec:null, lastEnemyKey:null, snapshots:[], summary:{ killed:{}, jenny:0, exp:0, cardsGained:[], itemsGained:[], healthLost:0, cardsLost:[], itemsLost:[] } };
  snapshot(state, 0);
...
async function claimRewards(env, user, character, exploration) {
  ...
  if (!exploration.rewardsPaid) {
    character = { ...character, health:exploration.final?.health || character.health, cards:Array.isArray(exploration.final?.cards) ? exploration.final.cards : (character.cards || []), inventory:Array.isArray(exploration.final?.inventory) ? exploration.final.inventory : (character.inventory || []), xp:clampInt(exploration.final?.xp, clampInt(character.xp)), jenny:clampInt(exploration.final?.jenny, clampInt(character.jenny)), activeExploration:null };
  } else character = { ...character, activeExploration:null };
  character = normalizeLevelXp(character); exploration.claimed = true; exploration.status = 'claimed'; exploration.rewardsPaid = true; exploration.rewardsPaidAt = new Date().toISOString();
  await saveCharacter(env, user, character); await saveExplore(env, user, exploration);
  return character;
}
...
export async function onRequestPost({ request, env }) {
  ...
  if (action === 'start') {
    ...
    const requestedMode = clean(data.mode).toLowerCase();
    const mode = requestedMode.includes('zetsu') ? 'zetsu' : 'scoperta';
