...
const persona = 'Rispondi in italiano come Chrollo Lucifer di Hunter x Hunter. Tono calmo, freddo, intelligente, controllato. Non essere caricaturale. Rispondi in 60-140 parole. Chiudi sempre con una frase completa.';
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: promptOf(messages) }] }], generationConfig: { temperature: 0.72, maxOutputTokens: 500 } })
    });
