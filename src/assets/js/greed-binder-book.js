(() => {
  ...
  const apiPost = async body => {
    const res = await fetch('/api/hxh-character', { method:'POST', headers:{ 'content-type':'application/json', authorization:`Bearer ${token()}` }, body:JSON.stringify(body), cache:'no-store' });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || 'Errore Binder Book');
    return data;
  };
  ...
  css.textContent = `
    .binder-book{position:relative;perspective:1400px;min-height:560px;padding:8px 0 0}.binder-head{display:flex;align-items:end;justify-content:space-between;gap:12px;margin:0 0 12px}.binder-head h2{margin:0!important;color:#ffe16a!important}.binder-count{color:#dfff73;font:900 12px/1.2 'Courier New',monospace;text-transform:uppercase}.binder-shell{position:relative;background:linear-gradient(90deg,#180802 0%,#441500 8%,#1a0700 50%,#441500 92%,#180802 100%);border:3px solid #ffe16a;box-shadow:10px 10px 0 rgba(0,0,0,.78),inset 0 0 34px rgba(255,225,106,.13);padding:18px;border-radius:10px;overflow:hidden}.binder-shell::before{content:"";position:absolute;left:50%;top:0;bottom:0;width:14px;transform:translateX(-50%);background:linear-gradient(90deg,rgba(0,0,0,.7),rgba(255,255,255,.16),rgba(0,0,0,.75));box-shadow:0 0 22px rgba(0,0,0,.9);z-index:1}.binder-page{position:relative;z-index:2;background:linear-gradient(135deg,rgba(255,246,200,.95),rgba(198,164,92,.92));border:2px solid #2a1200;border-radius:7px;min-height:430px;padding:16px;color:#120900;box-shadow:inset 0 0 28px rgba(70,28,0,.24);transform-origin:left center;animation:binderFlip .34s ease both}.binder-page.is-back{transform-origin:right center}.binder-page-title{display:flex;justify-content:space-between;gap:10px;align-items:center;border-bottom:2px solid rgba(42,18,0,.55);padding-bottom:8px;margin-bottom:12px;font:900 14px/1.2 'Courier New',monospace;text-transform:uppercase}.binder-page-title strong{font:900 24px/1 Impact,Haettenschweiler,'Arial Black',sans-serif;color:#481800}.binder-slots{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.binder-slot{position:relative;min-height:104px;border:2px solid rgba(42,18,0,.72);background:rgba(255,255,255,.38);box-shadow:inset 0 0 14px rgba(0,0,0,.12);padding:8px 8px 8px 74px;overflow:hidden}.binder-slot::before{content:attr(data-slot);position:absolute;left:7px;top:7px;width:52px;height:28px;display:grid;place-items:center;background:#170800;color:#ffe16a;font:900 12px/1 'Courier New',monospace;border-radius:3px}.binder-slot.is-free::before{background:#173000;color:#dfff73}.binder-slot.is-spell{border-color:#c99100;background:linear-gradient(135deg,rgba(255,244,180,.76),rgba(235,193,45,.44))}.binder-slot.is-spell::before{background:#7b4b00;color:#fff3a3}.binder-slot.is-filled{background:linear-gradient(135deg,rgba(255,255,255,.75),rgba(255,226,104,.72));box-shadow:inset 0 0 18px rgba(255,225,106,.5),0 0 12px rgba(255,225,106,.25)}.binder-card-art{position:absolute;left:8px;top:42px;width:54px;height:54px;border:2px solid rgba(42,18,0,.7);background:linear-gradient(135deg,#fff2a8,#c9872d);display:grid;place-items:center;box-shadow:inset 0 0 12px rgba(0,0,0,.18)}.binder-card-art svg{width:45px;height:45px;display:block}.binder-card-name{display:block;font:900 13px/1.14 Arial,Helvetica,sans-serif;color:#120900}.binder-card-type{display:block;margin-top:5px;font:800 10px/1.2 'Courier New',monospace;color:#5a2400;text-transform:uppercase}.binder-card-desc{display:block;margin-top:5px;font:700 11px/1.25 Arial,Helvetica,sans-serif;color:#331600}.binder-empty{display:block;font:800 12px/1.2 'Courier New',monospace;color:rgba(18,9,0,.55);text-transform:uppercase}.binder-card-actions{margin-top:7px}.binder-card-actions button{border:1px solid #120900;background:#1d5b00;color:#edffd5;font:900 10px/1 'Courier New',monospace;text-transform:uppercase;padding:6px 7px;cursor:pointer;box-shadow:2px 2px 0 rgba(0,0,0,.4)}.binder-card-actions button:disabled{opacity:.45;filter:grayscale(1);cursor:not-allowed}.binder-controls{display:flex;justify-content:space-between;gap:10px;margin-top:12px;flex-wrap:wrap}.binder-controls button,.binder-spell-toggle{border:2px solid #ffe16a;background:rgba(0,0,0,.68);color:#ffe16a;font:900 12px/1 'Courier New',monospace;text-transform:uppercase;padding:10px 12px;cursor:pointer;box-shadow:3px 3px 0 #000}.binder-spell-toggle{border-color:#fff3a3;color:#fff3a3;background:rgba(91,57,0,.78);width:100%;margin-top:10px}.binder-controls button:disabled{opacity:.35;cursor:not-allowed;filter:grayscale(1)}.binder-help{margin:12px 0 0;color:#f4ffe8;font:700 12px/1.35 Arial,Helvetica,sans-serif}.binder-help strong{color:#dfff73}.binder-help em{color:#fff3a3;font-style:normal;font-weight:900}.binder-msg{margin:10px 0 0;color:#dfff73;font:900 12px/1.3 'Courier New',monospace}.binder-msg.err{color:#ff7474}@keyframes binderFlip{0%{opacity:.35;transform:rotateY(-18deg) scale(.98)}100%{opacity:1;transform:rotateY(0) scale(1)}}@media(max-width:760px){.binder-book{min-height:500px}.binder-shell{padding:12px}.binder-page{min-height:390px;padding:12px}.binder-slots{grid-template-columns:1fr}.binder-slot{min-height:98px}.binder-head{display:block}.binder-count{display:block;margin-top:6px}}
  `;
  ...
  const cardDesc = card => card?.description || card?.descrizione || '';
  const artSvg = card => {
    const art = card?.art || '';
    if (art === 'pig' || /maiale/i.test(card?.name || card?.nome || '')) return `<svg viewBox="0 0 64 64" role="img" aria-label="Maiale"><ellipse cx="33" cy="36" rx="22" ry="15" fill="#f6a8b8" stroke="#36120d" stroke-width="3"/><circle cx="22" cy="24" r="8" fill="#f6a8b8" stroke="#36120d" stroke-width="3"/><circle cx="43" cy="24" r="8" fill="#f6a8b8" stroke="#36120d" stroke-width="3"/><ellipse cx="33" cy="38" rx="10" ry="7" fill="#ffc2ce" stroke="#36120d" stroke-width="3"/><circle cx="29" cy="38" r="1.8" fill="#36120d"/><circle cx="37" cy="38" r="1.8" fill="#36120d"/><circle cx="25" cy="31" r="2.2" fill="#120900"/><circle cx="41" cy="31" r="2.2" fill="#120900"/><path d="M48 44c7 0 8-8 2-8" fill="none" stroke="#36120d" stroke-width="3" stroke-linecap="round"/></svg>`;
    return `<svg viewBox="0 0 64 64" role="img" aria-label="Carta"><rect x="13" y="8" width="38" height="48" rx="4" fill="#fff4b8" stroke="#2a1200" stroke-width="4"/><path d="M19 20h26M19 30h26M19 40h18" stroke="#2a1200" stroke-width="4" stroke-linecap="round"/></svg>`;
  };
  const renderSlot = (i, card, slotClass, emptyText, label, isFree = false) => {
    const canMaterialize = !!card && isFree && !isSpell(card);
    return `<div class="binder-slot ${slotClass} ${card ? 'is-filled' : ''}" data-slot="${label}" data-binder-slot="${i}">${card ? `<div class="binder-card-art">${artSvg(card)}</div><span class="binder-card-name">${esc(card.name || card.nome || `Carta ${label}`)}</span><span class="binder-card-type">${esc(card.typeLabel || card.effectLabel || emptyText)}${cardRarity(card) ? ` · ${esc(cardRarity(card)).toUpperCase()}` : ''}</span>${cardDesc(card) ? `<span class="binder-card-desc">${esc(cardDesc(card))}</span>` : ''}${canMaterialize ? `<div class="binder-card-actions"><button type="button" data-materialize-card="${i}">Materializza</button></div>` : ''}` : `<span class="binder-empty">${emptyText}</span>`}</div>`;
  };
  const render = (message = '', isError = false) => {
    ...
    for (let i = start; i < end; i++) {
      if (isSpellMode) {
        ...
        slots.push(renderSlot(i, spells[i], 'is-spell', 'Slot incantesimo', label, false));
      } else {
        const card = normal[i];
        const isNumbered = i < numberedSlots;
        const label = String(i).padStart(3, '0');
        slots.push(renderSlot(i, card, isNumbered ? 'is-specific' : 'is-free', isNumbered ? 'Slot carta numerata' : 'Slot carta libera', label, !isNumbered));
      }
    }
  ...
  panel.addEventListener('click', async e => {
    ...
    const mat = e.target.closest('[data-materialize-card]');
    if (mat) {
      e.preventDefault();
      try {
        const data = await apiPost({ action:'materialize-card', slot:Number(mat.dataset.materializeCard) });
        character = data.character || character;
        window.dispatchEvent(new CustomEvent('greed-character-updated', { detail:character }));
        render(`${data.item?.name || data.item?.nome || 'Oggetto'} materializzato nell'inventario.`);
