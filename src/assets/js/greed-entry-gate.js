(() => {
  ...
  let busy = false;
  const keyFor = c => `greedEnteredV5:${c?.userId || c?.nome || 'player'}:${c?.createdAt || 'created'}`;
  ...
  fetch('/api/hxh-character', { headers:{ authorization:`Bearer ${token()}` }, cache:'no-store' })
    .then(res => res.ok ? res.json() : null)
    .then(data => handleCharacter(data?.character))
    .catch(() => {});
})();
