(() => {
  ...
  const keyFor = c => `greedEnteredV4:${c?.userId || c?.nome || 'player'}:${c?.createdAt || 'created'}`;
  ...
  const ensureButton = c => {
    ...
    if (!wrap) {
      ...
      wrap.innerHTML = '<button type="button" class="gi-param-enter is-locked" data-gi-param-enter disabled>ENTRA IN GREED ISLAND</button><p class="gi-param-enter-note is-locked" data-gi-param-enter-note>Spendi tutti i 10 punti per entrare.</p>';
    ...
    const locked = (c.setupPoints || 0) > 0 || !c.ready || localStorage.getItem(keyFor(c)) === '1';
    btn.disabled = locked;
    btn.classList.toggle('is-locked', locked);
    note.classList.toggle('is-locked', locked);
    note.textContent = locked ? `Punti rimasti: ${c.setupPoints || 0}. Spendi tutti i 10 punti per entrare.` : 'Parametri completati. Greed Island ti sta aspettando.';
  };
  const evaluate = async (incoming = null) => {
    ...
    lastCharacter = c;
    const card = document.querySelector('.param-card');
    if (card && ((c.setupPoints || 0) > 0 || (c.ready && localStorage.getItem(keyFor(c)) !== '1'))) {
      ensureButton(c);
      return;
    }
  ...
  const observer = new MutationObserver(() => {
    const card = document.querySelector('.param-card');
    if (card && lastCharacter) ensureButton(lastCharacter);
  });
