(() => {
  ...
  let busy = false;
  let lastReadyId = '';
  const keyFor = c => `greedEnteredV3:${c?.userId || c?.nome || 'player'}:${c?.createdAt || 'created'}`;
  ...
  const startWelcome = (c, speed = 'fast') => {
    ...
    document.body.classList.remove('has-param-setup','greed-param-enter-needed');
  ...
  const ensureEnterButton = c => {
    const card = document.querySelector('.param-card');
    if (!card || !c?.ready || localStorage.getItem(keyFor(c)) === '1') return;
    lastReadyId = `${c.userId || ''}:${c.createdAt || ''}`;
    ...
    if (!wrap) {
      ...
      wrap.innerHTML = '<p class="gi-param-enter-note">Parametri completati. Greed Island ti sta aspettando.</p><button type="button" class="gi-param-enter" data-gi-param-enter>ENTRA</button>';
      card.appendChild(wrap);
      wrap.querySelector('[data-gi-param-enter]').addEventListener('click', () => startWelcome(c, 'slow'));
    }
  };
  const evaluate = async (incoming = null) => {
    ...
    if (!c) return;
    if ((c.setupPoints || 0) > 0) return;
    if (!c.ready) return;
    const inSetupScreen = document.body.classList.contains('has-param-setup') || !!document.querySelector('.param-card');
    if (inSetupScreen && localStorage.getItem(keyFor(c)) !== '1') {
      ensureEnterButton(c);
      return;
    }
    if (localStorage.getItem(keyFor(c)) !== '1' && lastReadyId !== `${c.userId || ''}:${c.createdAt || ''}`) startWelcome(c, 'fast');
  };
