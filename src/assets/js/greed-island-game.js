(() => {
  ...
  style.textContent = `
    .greed-island-page{background:linear-gradient(rgba(0,0,0,.1),rgba(0,0,0,.34)),url('/assets/img/greed.png') center/cover no-repeat!important}.greed-game{background:none!important;overflow:hidden;touch-action:none;cursor:grab}.greed-game.is-dragging{cursor:grabbing}.map-world{position:absolute;inset:0;z-index:6;background:linear-gradient(rgba(0,0,0,.04),rgba(0,0,0,.18)),url('/assets/img/greed.png') center/cover no-repeat;transform-origin:50% 50%;will-change:transform}.map-hint{position:fixed;right:14px;top:18px;z-index:30;color:#fff;font:400 12px/1.2 Arial,sans-serif;text-shadow:1px 1px 0 #000;opacity:.72}
    .hxh-welcome{position:fixed;top:16px;left:50%;transform:translateX(-50%);z-index:30;color:#fff;font:500 28px/1.15 Arial,Helvetica,sans-serif;letter-spacing:0;text-shadow:1px 1px 0 #000}.hxh-welcome strong{font-weight:600;color:#fff}
    ...
    .location-display,.level-display,.jenny-display{z-index:30;color:#fff;font:400 15px/1.2 Arial,Helvetica,sans-serif!important;text-transform:none!important;letter-spacing:0!important;text-shadow:1px 1px 0 #000;background:transparent!important;border:0!important;box-shadow:none!important;padding:0!important}.location-display{max-width:150px;white-space:normal;word-break:normal;overflow-wrap:normal}.location-display.long strong{display:block}.location-display strong,.level-display strong,.jenny-display strong{color:#fff;font-weight:500}.level-display{position:fixed;top:22px;left:calc(var(--side,44px) + 288px)}.jenny-display{position:fixed;top:44px;left:calc(var(--side,44px) + 288px)}
    ...
    @media(max-width:760px){.hxh-welcome{top:146px;width:calc(100vw - var(--side,38px) - 42px);text-align:center;font-size:22px}.test-levelup{top:190px}.level-display{top:104px;left:calc(var(--side,38px) + 14px)}.jenny-display{top:126px;left:calc(var(--side,38px) + 14px)}.param-card{width:calc(100vw - var(--side,38px) - 28px)}.stat-mini-grid{grid-template-columns:1fr}.map-hint{display:none}}
  `;
  ...
  const refreshMap = () => {
    ...
    if (locationLabel) {
      ...
      locationLabel.closest('.location-display')?.classList.toggle('long', loc.length > 9);
    }
